...
$("#formulario").steps({
    ...
    onStepChanging: function (event, currentIndex, newIndex) {
        ...
        console.log('onStepChanging : '+String(currentIndex) + '  ' + String(newIndex));
        if (!valido) return 0;
                        ...
                        .then(e => {                                                
                            if (e.continua) {
                                ...
                                if (e.message == 'PE') {
                                    ...
                                    mensaje = 'Por favor inice sesion nuevamente';
                                }
                                statusExecValidPerYFrec = 1;
                                return Swal.fire({
                                    icon: 'warning',
    ...
    onStepChanged: function (event, currentIndex, priorIndex) {
        console.log('onStepChanged : '+String(currentIndex) + '  ' + String(priorIndex));
        var todo = $('#btnsave');
        todo.remove();
        /*if (currentIndex == 1) {
            actCatSel();
        }
        else if (currentIndex == 2) {//MODIFICADO POR CAMBIOS
            actCanYZonas();
        ...
        //else if (currentIndex == 3) {//MODIFICADO POR CAMBIOS
        if (currentIndex == 1 ) {
            var todo = $('#btnswizard');
            ...
            var html = `<li id="btnsave">
                        <a style="background-color:rgb(229, 96, 83);" onclick="SaveEstado1()" > Pre-Guardado</a>
                        </li>`;
            todo.append(html);
            ...
            //alert(priorIndex);//0
            if (currentIndex > priorIndex) {
                var iResponse = fValidateTblLoad();
                //alert(iResponse);
                if (iResponse == 1) {
                    SetTblPlanificacion1();
                    return 1;
                } else if (iResponse == 0) {
                    return 1;
                }
